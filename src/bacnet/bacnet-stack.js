...
let bacstack = new bacnet({
  port: store.get("port"),
});

bacstack.on("whoIs", (data) => {
  if (data.lowLimit && data.lowLimit > store.get("deviceId")) return;
  if (data.highLimit && data.highLimit < store.get("deviceId")) return;
 ...
 */
log.info("Node BACstack Device started");

module.exports = bacstack;
